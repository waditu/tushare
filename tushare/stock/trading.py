    ...
    for _ in range(retry_count):
        ...
        try:
            ...
            if len(js['record'][0]) == 14:
                cols = ct.INX_DAY_PRICE_COLUMNS
            df = pd.DataFrame(js['record'][-num if num else None:], columns=cols)
...
def get_stock_now_market(code):
    '''
    个股当前行情
    来源: http://hq.sinajs.cn/list=sz000001
    内容:
    Parameters
    --------
        code:股票代码
    Return
    --------
        var hq_str_sh601006="大秦铁路, 27.55, 27.25, 26.91, 27.55, 26.20, 26.91, 26.92,22114263, 589824680, 4695, 26.91, 57590, 26.90, 14700, 26.89, 14300,26.88, 15100, 26.87, 3100, 26.92, 8900, 26.93, 14230, 26.94, 25150, 26.95, 15220, 26.96, 2008-01-11, 15:05:32, 00";
        ...
        30：”2008-01-11″，日期；
        31：”15:05:32″，时间；
        32：”00″，状态(00正常, 03停牌)    数据为空表示退市
    '''
    symbol = _code_to_symbol(code)
    url = 'http://hq.sinajs.cn/list=%s' % (symbol)
    data = urlopen(Request(url), timeout=10).read().decode('GBK')
    g = re.search('="(.+)"', data)
    return g.group(1).split(',') if g else []