...
REPORT_URL = '%s%s/q/go.php/vFinanceAnalyze/kind/mainindex/%s?s_i=&s_a=&s_c=&reportdate=%s&quarter=%s&p=%s&num=%s&order=%s%%7C1'   # 增加排序功能 &order=%s%%7C1
REPORT_ORDERBY= {'default':'code','code':'code', 'eps':'eps','bvps':'bvps', 'roe':'roe', 'netprofit':'netprofit','net_profit':'netprofit','net_profits':'netprofit','cps':'cps','epcf':'cps'}  # 排序检查用
FORECAST_URL = '%s%s/q/go.php/vFinanceAnalyze/kind/performance/%s?s_i=&s_a=&s_c=&s_type=&reportdate=%s&quarter=%s&p=%s&num=%s'
PROFIT_URL = '%s%s/q/go.php/vFinanceAnalyze/kind/profit/%s?s_i=&s_a=&s_c=&reportdate=%s&quarter=%s&p=%s&num=%s&order=%s%%7C1'   # 增加排序功能 &order=%s%%7C1
PROFIT_ORDERBY = {'default':'roe','roe':'roe', 'netprofitmargin':'netprofitmargin','net_profit_ratio':'netprofitmargin','profitmargin':'profitmargin','gross_profit_rate':'profitmargin','netprofit':'netprofit','net_profits':'netprofit','eps':'eps','business_income':'income', 'income':'income','mips':'mips','bips':'mips'}   # 排序检查用
OPERATION_URL = '%s%s/q/go.php/vFinanceAnalyze/kind/operation/%s?s_i=&s_a=&s_c=&reportdate=%s&quarter=%s&p=%s&num=%s&order=%s%%7C2' # 增加排序功能 &order=%s%%7C2
OPERATION_ORDERBY = {'default':'yszkzzl','yszkzzl':'yszkzzl','arturnover':'yszkzzl','yszkzzts':'yszkzzts','arturndays':'yszkzzts','chzzl':'chzzl','inventory_turnover':'chzzl','chzzts':'chzzts ','inventory_days':'chzzts','ldzczzl':'ldzczzl','urrentasset_turnover':'ldzczzl','ldzczzts':'ldzczzts','currentasset_days':'ldzczzts'}   # 排序检查用
GROWTH_URL = '%s%s/q/go.php/vFinanceAnalyze/kind/grow/%s?s_i=&s_a=&s_c=&reportdate=%s&quarter=%s&p=%s&num=%s&order=%s%%7C2'   # 增加排序功能 &order=%s%%7C1
GROWTH_ORDERBY = {'default':'mainincome','mainincome':'mainincome','mbrg':'mainincome','netprofit':'netprofit','nprg':'netprofit','netassets':'netassets','nav':'netassets','totalassets':'totalassets','targ':'totalassets'}  # 排序检查用
DEBTPAYING_URL = '%s%s/q/go.php/vFinanceAnalyze/kind/debtpaying/%s?s_i=&s_a=&s_c=&reportdate=%s&quarter=%s&p=%s&num=%s&order=%s%%7C2'  # 增加排序功能 &order=%s%%7C2
DEBTPAYING_ORDERBY = {'default':'current', 'current':'current','currentratio':'current','quick':'quick','quickratio':'quick','cash':'cash','cashratio':'cash','icr':'icr','icratio':'icr','equity':'equity','sheqratio':'equity'}  # 排序检查用
CASHFLOW_URL = '%s%s/q/go.php/vFinanceAnalyze/kind/cashflow/%s?s_i=&s_a=&s_c=&reportdate=%s&quarter=%s&p=%s&num=%s&order=%s%%7C2'    # 增加排序功能 &order=%s%%7C2
CASHFLOW_ORDERBY = {'default':'cashflow2income','cashflow2income':'cashflow2income','cf_sales':'cashflow2income','cashflowrepay':'cashflowrepay','rateofreturn':'cashflowrepay','cashflow2netprofit':'cashflow2netprofit','cf_nm,':'cashflow2netprofit','cashflow2debt':'cashflow2debt','cf_liabilities':'cashflow2debt','cashflow':'cashflow','cashflowratio':'cashflow'}     # 排序检查用
...
# 增加排序检查
def _check_orderby(orderby, orderitems):
    orderby = orderby.lower()
    if orderby in orderitems:
        return orderitems[orderby]
    else:
        return orderitems['default']